...
class Paraphraser(object):
    ...
    def greedy_paraphrase(self, sentence):
        ...
        return self.infer(0, sentence, self.idx_to_word, 0.15, 1)
...
#     print(p)
    paraphrases = paraphraser.sample_paraphrase(source_sentence, sampling_temp=0.15, how_many=1)
    for i in range(10):
        paraphrases = paraphraser.sample_paraphrase(source_sentence, sampling_temp=0.3, how_many=10)
        print(type(paraphrases))
        for j, paraphrase in enumerate(paraphrases):
            print("Paraph #{}: {}".format(j, paraphrase))
#     paraphrases = paraphraser.infer(0, source_sentence, )
